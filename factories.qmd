...
## Заводы стоят на производство такого колличества:
...
print(show_production_rates(production_rates))
```


## В каких количествах потребляется:
...
def show_recipe(recipe):
    summary = ""
    for i in recipe:
        summary = summary + " " + i.value +": "+ str(recipe[i]) + " "
    return summary

def show_cons_rate(cons_rates):
    summary = ""
    for i in cons_rates:
        summary = summary + "**"+i.value +"**"+ "| " + show_recipe(cons_rates[i]) + "\n\n"
    return summary

cons_rate = consumption_rate(production_on_floor, production_floors)

print(show_cons_rate(cons_rate))
```

## Разница потребления и производства
...
def show_diff(diff):
    summary = ""
    for i in diff:
        summary = summary + "**"+i.value+"**: "+str(diff[i])+"\n\n"
    return summary

import pandas as pd

#f = pd.DataFrame.from_dict(diff_c_p, orient='index', columns=["name"])
#print(df.to_markdown(index=False))
#print(show_diff(diff_c_p))
```
...
d = diff_c_p
headers = ["Name", "Разница"]
#Markdown(tabulate([(k.value,get_number_with_color(v))  for k,v in d.items()], headers = headers))  
#Markdown(get_number_with_color(-12))
```
...
Markdown(tabulate([(k.value, show_recipe(v))  for k,v in recipes.items()], headers = headers))  

#show_recipe(recipes[Item.Automated_wiring])
```
